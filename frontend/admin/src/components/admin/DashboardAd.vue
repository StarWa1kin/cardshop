...
const visibleItems = computed(
  () => adData.value?.items.filter(isVisible) ?? [],
);

// Banner carousel: 2 items per page on md+, 1 on mobile
const bannerNeedCarousel = computed(
  () => props.layout === "banner" && visibleItems.value.length > 2,
);
const itemsPerPage = 2; // md+ shows 2, mobile shows 1 but we paginate by 2
const totalPages = computed(() =>
  Math.ceil(visibleItems.value.length / itemsPerPage),
);
  ...
  <template v-if="visible && adData">
    ...
    <template v-if="layout === 'banner'">
      ...
      >
        ...
        >
          ...
          <div
            class="absolute right-1.5 top-1.5 text-xs text-muted-foreground/40"
          >
            {{ t("admin.dashboard.ad.label") }}
          </div>
        </div>
      </div>
      ...
      >
        <div class="overflow-hidden">
          <div
            class="grid gap-3 grid-cols-1 md:grid-cols-2 transition-opacity duration-300"
          >
            <div
    ...
    <template v-else-if="layout === 'notice'">
      <div
        ...
        :class="
          item.theme === 'highlight' ? 'border-primary/20 bg-primary/5' : ''
        "
      >
        <img
          v-if="item.icon"
      ...
      <div class="rounded-xl border border-border bg-card">
        ...
        >
          <span class="text-xs font-medium text-muted-foreground">{{
            t("admin.dashboard.ad.sponsored")
          }}</span>
          <span class="text-[10px] text-muted-foreground/40">{{
            t("admin.dashboard.ad.label")
          }}</span>
        ...
        <div
          class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-x-4 gap-y-1 px-4 py-2.5"
        >
          <div
        ...
        <div v-if="compactHasMore" class="border-t border-border px-4 py-2">
          ...
          >
            {{
              compactExpanded
                ? t("admin.dashboard.ad.showLess")
                : t("admin.dashboard.ad.showAll", {
